...
// Lightweight in-browser cache key (until a server endpoint for profile PUT is implemented)
const LS_PROFILE_KEY = 'therapist:profileDraft:v1'

function loadDraft(): ProfileDraft {
  if (typeof localStorage === 'undefined') return { name:'', displayName:'', timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC', email:'', notifyMessageEmail:true, notifyDailySummary:false, photoFile:null, uploading:false, photoUrl:null }
  try {
    const raw = localStorage.getItem(LS_PROFILE_KEY)
    if (!raw) return { name:'', displayName:'', timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC', email:'', notifyMessageEmail:true, notifyDailySummary:false, photoFile:null, uploading:false, photoUrl:null }
    const d = JSON.parse(raw)
    return { name:d.name||'', displayName:d.displayName||'', timezone:d.timezone||Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC', email:d.email||'', notifyMessageEmail:d.notifyMessageEmail!==false, notifyDailySummary:!!d.notifyDailySummary, photoFile:null, uploading:false, photoUrl:d.photoUrl||null }
  } catch { return { name:'', displayName:'', timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC', email:'', notifyMessageEmail:true, notifyDailySummary:false, photoFile:null, uploading:false, photoUrl:null } }
}
...
export default function TherapistSettingsPage() {
  ...
  useEffect(()=>{ try { const { photoFile, uploading, ...persist } = draft; localStorage.setItem(LS_PROFILE_KEY, JSON.stringify(persist)) } catch {} }, [draft])

  async function saveProfile() {
    setStatus('Saving…')
    // Placeholder: simulate network latency
    await new Promise(r=>setTimeout(r, 600))
    setStatus('Saved (local only — server endpoint not yet implemented)')
  }
  ...
  async function handlePhoto(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      // Upload to existing media endpoint (re-using general upload) if available
      ...
      if (res.ok) {
        ...
        setStatus('Photo uploaded (not yet committed to profile server record).')
      } else {
    ...
    <main className="mx-auto max-w-3xl p-2 sm:p-4 space-y-8">
      ...
      <section className="rounded border bg-white p-4 space-y-4">
        ...
        <label className="flex items-start gap-3 text-sm">
          ...
          <span>
            ...
            <span className="block text-gray-600 text-xs">Digest of shifts, events, and unread message counts (future feature).</span>
          </span>
        </label>
      </section>
      ...
      <section className="rounded border bg-white p-4 space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          <button type="button" className="rounded border px-3 py-1 text-sm" onClick={()=>{ localStorage.removeItem(LS_PROFILE_KEY); setDraft(loadDraft()); setStatus('Draft reset') }}>Reset draft</button>
      ...
      <div className="flex items-center justify-between border-t pt-4">
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={() => {
            try {
              localStorage.removeItem('tour:auto:THERAPIST');
              localStorage.removeItem('tour:skip:THERAPIST');
            } catch {}
            fetch('/api/tour/skip?role=THERAPIST', { method: 'DELETE' }).catch(()=>{})
