...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!isProd) {
      // Dev fallback to avoid 500s while DB is not ready
      if (!studentId) return NextResponse.json({ students: [] })
      return NextResponse.json({ student: null, participants: [], messages: [] })
    }
