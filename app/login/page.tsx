...
export default function LoginPage() {
  ...
  {error && <div className="rounded bg-red-50 border border-red-200 text-red-800 px-3 py-2 text-sm">{error}</div>}
        ...
        <button onClick={handlePasswordLogin} className="w-full rounded bg-brand-600 text-white px-4 py-2">Sign in</button>
        <div className="text-center text-xs text-gray-500">or</div>
        <button onClick={handleSSO} className="w-full rounded border px-4 py-2">Continue with SSO</button>
        <div className="text-xs text-gray-500">Dev shortcuts</div>
        <div className="flex gap-2">
          <button onClick={() => handleDevSSO('THERAPIST')} className="flex-1 rounded border px-3 py-2">Dev SSO Therapist</button>
          <button onClick={() => handleDevSSO('PARENT')} className="flex-1 rounded border px-3 py-2">Dev SSO Parent</button>
        </div>
      </div>
